...
export class haveIBeenPwned {
  /**
   * Check against the haveibeenpwned.com API to see if your password is safe
   * @param password The password you want to check.
   */
  static async CheckPassword(password: string) {
    if (password || password == '') {
      // do the sha1
      const hashed = new sha1(password).hashed;
      const firstFive = hashed.substring(0,5);
      try {
        const uri = `https://api.pwnedpasswords.com/range/${firstFive}`;       
        let request = await fetch(uri);
        let results = await request.text();
        // ex of return value: 0018A45C4D1DEF81644B54AB7F969B88D65:1
        // parse the results and see if we have a match
        let result = results.split('\n').map((val) => {
          return val.split(':')[0];
        }).find(x => firstFive + x == hashed);
...
export class sha1 {
  ...
  /**
   * The hash value produced by sha1
   */
  get hashed(): string {
    return this._hashed;
  }
  ...
  /**
   * Constructor for sha1
   * @param text The text you want to hash
   */
  constructor (text: string) {
    this._hashed = this.hash(text);
  }
  ...
  private hash(msg: string) {
    ...
    let tempf = this.cvt_hex(H0) + this.cvt_hex(H1) + this.cvt_hex(H2) + this.cvt_hex(H3) + this.cvt_hex(H4);
    return tempf.toUpperCase();
  }
